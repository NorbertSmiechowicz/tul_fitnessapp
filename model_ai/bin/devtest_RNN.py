...
def mnist(device, train, augment=False):
    ...
    if augment:
        ...
        for img in data_zero:
            ...
            augmented_img_tensor = transforms.ToTensor()(augmented_img)
...
def main():
    ...
    """
    data, sequence_length = mnist(device=device, train=True)
    train = True
    ...
    if train:
        ...
        for epoch in range(num_epochs):
            for batch in data_loader:
                ...
                h = (torch.zeros(1, images.size(0), 128).to(device),
                    torch.zeros(1, images.size(0), 128).to(device))
                """
                h in RNN Model
